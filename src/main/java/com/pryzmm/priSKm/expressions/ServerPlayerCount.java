...
public class ServerPlayerCount extends SimpleExpression<Number> {
    private Expression<Server> server;

    static {
        Skript.registerExpression(ServerPlayerCount.class, Number.class,
                ExpressionType.PROPERTY, "player count of %server%", "%server%['s] player count");
    }
    ...
    protected Number[] get(Event event) {
        ...
        Number result = srv.getPlayerCount();
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "player count of " + server.toString(event, debug);
    }
}