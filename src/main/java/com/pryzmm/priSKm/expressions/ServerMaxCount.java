...
public class ServerMaxCount extends SimpleExpression<Number> {
    private Expression<Server> server;

    static {
        Skript.registerExpression(ServerMaxCount.class, Number.class,
                ExpressionType.PROPERTY, "max[imum] player[s] [count] of %server%", "%server%['s] max[imum] player[s] [count]");
    }
    ...
    protected Number[] get(Event event) {
        ...
        Number result = srv.getMaxPlayers();
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "max players of " + server.toString(event, debug);
    }
}